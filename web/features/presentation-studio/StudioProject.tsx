      ...
      <section className="min-w-0 space-y-4">
        ...
        {tab === "pages" && <>
          ...
          <div className="rounded-2xl border border-violet-100 bg-white p-4 sm:p-5">
            ...
            <SlidePreview project={id} revision={project.current.id} page={currentPage} />
            <div className="my-4 flex flex-wrap gap-2"><button className={button} disabled={currentPage <= 1} onClick={() => setPage(currentPage - 1)}>이전</button><button className={button} disabled={currentPage >= slides.length} onClick={() => setPage(currentPage + 1)}>다음</button><span className="flex-1" /><button className={button} disabled={disabled || currentPage <= 1} onClick={() => void move(-1)}>앞으로 이동</button><button className={button} disabled={disabled || currentPage >= slides.length} onClick={() => void move(1)}>뒤로 이동</button></div>
          ...
          <div className="flex flex-wrap gap-2">{slides.map(s => <button key={s.number} onClick={() => setPage(s.number)} title={s.title} aria-current={currentPage === s.number ? "page" : undefined} className={`h-10 min-w-10 rounded-lg border px-2 text-xs ${currentPage === s.number ? "border-violet-500 bg-violet-700 text-white" : "border-violet-100 bg-white text-slate-600"}`}>{project.protected_slides.includes(s.number) ? "🔒" : ""}{s.number}</button>)}</div>
        </>}
